...
export default function ResultsPage() {
  ...
  const UnavailableState = ({ icon: Icon, title, reason }: { icon: any, title: string, reason: string }) => (
    <div className="glass rounded-[2.5rem] p-8 md:p-12 flex flex-col items-start text-left gap-6 border-dashed border-primary/40 animate-in fade-in duration-700 w-full">
      ...
      <div className="space-y-3 w-full">
        ...
        <div className="flex items-center gap-2 text-[10px] font-black text-primary-950 uppercase tracking-widest bg-primary/10 px-5 py-2.5 rounded-full border border-primary/20 w-fit">
    ...
    <div className="min-h-screen pb-24 bg-background selection:bg-accent/30">
      <div className="max-w-5xl mx-auto p-4 md:p-10 space-y-10">
        ...
        <header className="flex items-center justify-between animate-in fade-in slide-in-from-top-4 duration-700">
          <Button variant="ghost" size="icon" onClick={() => router.push('/scan')} className="rounded-full glass hover:bg-white/40 border-primary/20">
          ...
          <div className="text-right">
            ...
            <h1 className="text-2xl font-headline font-bold text-primary tracking-tighter uppercase">Rapport Expert</h1>
          </div>
        </header>

        <section className="animate-in zoom-in-95 duration-1000">
          <Card className="glass rounded-[3rem] p-6 md:p-12 relative overflow-hidden border-primary/30 shadow-xl bg-white/40">
            <div className="flex flex-col gap-10">
              <div className="space-y-6 text-center md:text-left">
                ...
                <div className="flex items-center justify-center md:justify-start gap-6">
                  <div className="glass p-5 rounded-[2rem] flex flex-col items-center min-w-[100px] border-primary/40 bg-white/60">
                    <span className="text-4xl font-black text-primary">{data.globalScore || 0}</span>
                    <span className="text-[9px] font-black text-primary/80 uppercase tracking-widest">Bio-Score</span>
                  </div>
                  <div className={cn(
              ...
              <div className="flex flex-col gap-4">
                {(data.quickLook || []).map((item, i) => (
                   <div key={i} className="glass p-6 rounded-3xl border-primary/20 flex flex-col items-start gap-4 bg-white/60 w-full">
                      <div className="flex flex-col">
                        <p className="text-[10px] font-black text-primary/80 uppercase tracking-widest">{item.name}</p>
                        <span className="text-2xl font-black text-primary-950">{item.level}</span>
                      </div>
                      <Badge variant="outline" className="text-xs border-primary/30 px-4 py-2 font-bold rounded-xl bg-primary/5 text-primary-950 w-full text-left leading-relaxed shadow-inner">
                        {item.benefit}
                      </Badge>
                   </div>
                ))}
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full space-y-8">
          <TabsList className="grid w-full grid-cols-4 h-16 rounded-[2rem] glass p-2 gap-2 border-primary/30 shadow-md">
            ...
            <TabsTrigger value="symbiosis" className="rounded-[1.5rem] data-[state=active]:bg-emerald-700 data-[state=active]:text-white font-black transition-all">
              <Globe size={18} className="shrink-0" />
              <span className="hidden md:inline text-[10px] uppercase tracking-widest ml-2">Symbiose</span>
            </TabsTrigger>
            <TabsTrigger value="alchemy" className="rounded-[1.5rem] data-[state=active]:bg-amber-600 data-[state=active]:text-white font-black transition-all">
          ...
          <div className="min-h-[400px] animate-in slide-in-from-bottom-8 duration-700">
            <TabsContent value="vitality" className="space-y-8 mt-0">
              <div className="flex flex-col gap-8">
                <Card className="glass rounded-[3rem] p-8 md:p-12 border-primary/30 bg-white/60 shadow-lg relative overflow-hidden">
                   <h3 className="text-[10px] font-black uppercase tracking-[0.3em] text-primary mb-8 flex items-center gap-2">
                      <Target className="text-primary shrink-0" size={16} /> Budget Énergétique
                   </h3>
                   {data.caloricAnalysis ? (
                     <div className="space-y-10">
                        <div className="flex flex-col gap-1">
                          <span className="text-5xl md:text-7xl font-black text-primary-950 leading-none">{data.caloricAnalysis.caloriesPerPortion} <span className="text-sm font-bold text-primary/60 uppercase tracking-widest">kcal</span></span>
                          <p className="text-[10px] font-black uppercase tracking-widest text-primary/60 mt-2">Portion: {data.caloricAnalysis.estimatedPortion}</p>
                        </div>
                        <div className="space-y-4">
                           ...
                           <Progress value={data.caloricAnalysis.dailyBudgetContribution} className="h-4 bg-primary/10" />
                        </div>
                        <div className="p-8 rounded-[2.5rem] bg-primary/5 border border-primary/20 space-y-6">
                           <h4 className="text-[10px] font-black text-primary uppercase tracking-widest">Verdict de Qualité</h4>
                           <div className="flex flex-col gap-6">
                              ...
                              <p className="text-lg md:text-2xl font-bold leading-relaxed italic text-primary-950 border-l-4 border-primary/40 pl-8">
                ...
                <div className="glass p-8 md:p-12 rounded-[3rem] border-primary/30 bg-white/60 relative overflow-hidden shadow-lg">
                ...
                <div className="flex flex-col gap-4">
                   {(data.bonusTips?.healthBenefits || []).map((benefit, i) => (
                     <div key={i} className="glass p-8 rounded-[2.5rem] border-primary/20 flex flex-col items-start gap-4 bg-white/60 shadow-sm w-full">
            ...
            <TabsContent value="molecular" className="space-y-10 mt-0">
              <div className="flex flex-col gap-6">
                <div className="flex items-center gap-3 px-2">
                  <Gem className="text-primary w-6 h-6 shrink-0" />
                ...
                {data.molecularTreasures && data.molecularTreasures.length > 0 ? (
                  data.molecularTreasures.map((treasure, idx) => (
                    <Card key={idx} className="glass rounded-[2.5rem] p-8 md:p-10 border-primary/20 bg-white/60 shadow-sm flex flex-col items-start gap-5 w-full">
                      ...
                      <p className="text-sm md:text-lg text-primary-950 font-medium leading-relaxed italic border-l-2 border-primary/20 pl-6">"{treasure.description}"</p>
                    </Card>
                  ))
                ) : (
                  <UnavailableState 
              ...
              {data.scientificAlerts && data.scientificAlerts.length > 0 && (
                <div className="flex flex-col gap-6">
                  ...
                  {data.scientificAlerts.map((alert, idx) => (
                    <div key={idx} className="glass border-red-700/30 bg-red-600/5 p-8 md:p-10 rounded-[2.5rem] flex flex-col items-start gap-6 shadow-sm w-full">
                      <div className="bg-red-700 text-white p-4 rounded-2xl shrink-0 shadow-lg shadow-red-700/30">
                        <AlertTriangle size={24} />
                      </div>
                      <div className="space-y-4 w-full">
                        <Badge className="bg-red-700 text-white text-[10px] font-black tracking-widest py-1.5 px-6 uppercase rounded-full border-none">
            ...
            <TabsContent value="symbiosis" className="space-y-8 mt-0">
               {data.ecoIntelligence ? (
                 <Card className="glass border-emerald-700/40 rounded-[3rem] p-8 md:p-12 bg-white/60 shadow-lg flex flex-col gap-10 w-full">
                    <div className="flex flex-col items-start gap-8 w-full">
                       <div className="flex items-center gap-6">
                          <div className="w-24 h-24 rounded-[2.5rem] bg-emerald-700 text-white flex items-center justify-center text-5xl font-black shadow-xl shrink-0">
                             {data.ecoIntelligence.ecoScore || 0}
                          </div>
                          <div className="space-y-1">
                             <p className="text-[10px] font-black uppercase tracking-[0.3em] text-emerald-800">Impact Planétaire</p>
                       ...
                       <div className="flex flex-wrap gap-3">
                          {(data.ecoIntelligence.footprintTags || []).map((tag, i) => (
                             <Badge key={i} className="bg-emerald-700 text-white border-none text-[10px] font-black tracking-widest px-6 py-3 rounded-full uppercase shadow-sm">
                                {tag.toUpperCase()}
                             </Badge>
                          ))}
                       </div>
                    </div>
                    <div className="space-y-8 bg-emerald-700/10 p-8 rounded-[2.5rem] border border-emerald-700/20 shadow-inner w-full">
               ...
               <div className="flex flex-col gap-8 w-full">
                  {data.bonusTips?.expressRecipe ? (
                    <Card className="rounded-[3rem] bg-primary text-white p-8 md:p-16 border-none shadow-2xl relative overflow-hidden flex flex-col gap-10 w-full">
                       <div className="relative z-10 space-y-8">
                          <Badge className="bg-white text-primary border-none text-[10px] font-black tracking-widest px-8 py-3 rounded-full uppercase">Alchimie Culinaire</Badge>
                          <h3 className="text-3xl md:text-6xl font-headline font-bold leading-none">{data.bonusTips.expressRecipe.name}</h3>
                          <div className="space-y-4">
                             <p className="text-[10px] font-black uppercase tracking-[0.3em] text-white/80">Ingrédients Synergiques :</p>
                             <div className="flex flex-wrap gap-3">
                                {data.bonusTips.expressRecipe.ingredients.map((ing, i) => (
                                  <div key={i} className="bg-white/20 backdrop-blur-md px-6 py-3 rounded-2xl text-[12px] font-black border border-white/30 shadow-sm">
                                     {ing}
                                  </div>
                                ))}
                             </div>
                  ...
                  <div className="flex flex-col gap-6 w-full">
                     ...
                     {(data.bonusTips?.practicalTips || []).map((tip, i) => (
                        <div key={i} className="glass p-8 rounded-[2.5rem] flex flex-col items-start gap-5 bg-white/60 border-primary/20 shadow-sm w-full">
